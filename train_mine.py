...
# New args:
parser.add_argument('--target_ids', default=206, type=int,
...
if dataset == 'sysu':
    trainset = SYSUData_DA(data_path, num_ids=args.target_ids, transform=transform_train)
    ...
    # Making the test set RegDB
    # query_img, query_label = process_test_regdb('./RegDB/RegDB/', trial=args.trial, modal='visible')
    # gall_img, gall_label = process_test_regdb('./RegDB/RegDB/', trial=args.trial, modal='thermal')

elif dataset == 'regdb':
    # training set
    # trainset = RegDBData(data_path, args.trial, transform=transform_train)
    trainset = RegDBData_DA(data_path, args.trial, num_ids=args.target_ids, transform=transform_train)
    # generate the idx of each person identity
    color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)
...
# weights = [0.32, 0.35, 0.38, 0.41, 0.44]
# weights = [0.1, 0.3, 0.5, 0.7, 1, 1.5, 2]
weights = [0]
for weight_hdmmd in weights:
    print(f"\n\n\n------------ Now training for weight {weight_hdmmd} -------------\n\n")
    # training
    ...
    print('~~~ For weight {} ~~~\n\nBest Epoch [{}], Rank-1: {:.2%} |  mAP: {:.2%}| mINP: {:.2%}'.format(weight_hdmmd, best_epoch, best_acc, best_mAP, best_mINP))