...
parser.add_argument('--num_class', default=-1, type=int, help='Number of source+target classes')
...
# SYSU best
# model_path = 'save_model/margin_mmd1.40_sysu_c_tri_pcb_off_w_tri_2.0_share_net3_base_gm10_k4_p4_lr_0.1_seed_0_best.t' 

# SYSU40
# model_path = 'save_model/da_sysu40_sweep_source_sysu_c_tri_pcb_off_w_tri_2.0_share_net3_base_gm10_k4_p4_lr_0.1_seed_0_best.t'

# RegDB Best
# model_path = 'save_model/m3d_dmmd_regdb206_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_base_gm10_k4_p4_lr_0.1_seed_0_trial_1_best.t'

# RegDB AGW
# model_path = 'save_model/da_regdb40_sweep_hdmmd_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_agw_k4_p4_lr_0.1_seed_0_trial_1_best.t'

# RegDB AGW RegDB40 only Target
# model_path = 'save_model/agw_regdb40_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_agw_k4_p4_lr_0.1_seed_0_trial_1_best.t'

# RegDB 82
# model_path = 'save_model/exp_da_regdb82_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_agw_k4_p4_lr_0.1_seed_0_trial_1_best.t'

# RegDB da
# model_path = 'save_model/exp_da_regdb40_hdmmd_sweep_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_agw_k4_p4_lr_0.1_seed_0_trial_1_best.t'

# model_path = 'save_model/exp_da_regdb_sweep_ids_regdb_c_tri_pcb_off_w_tri_2.0_share_net3_agw_k4_p4_lr_0.1_seed_0_trial_1_best.t'
...
# Define a function to sample features
def sample_features(query_feat_fc, gall_feat_fc, query_label, gall_label, num_ids=10, num_samples=10):
    sampled_query_feats = []
    sampled_gall_feats = []
    sampled_query_labels = []
    sampled_gall_labels = []

    unique_ids = np.unique(query_label)  # Get unique identities

    # Sample num_ids different identities
    np.random.seed(0)
    selected_ids = np.random.choice(unique_ids, size=num_ids, replace=False)

    for identity in selected_ids:
        # Get indices for the current identity in query and gallery
        query_indices = np.where(query_label == identity)[0]
        gall_indices = np.where(gall_label == identity)[0]

        selected_query_indices = np.random.choice(query_indices, size=num_samples, replace=False)
        selected_gall_indices = np.random.choice(gall_indices, size=num_samples, replace=False)

        # Append sampled features and labels
        sampled_query_feats.append(query_feat_fc[selected_query_indices])
        sampled_gall_feats.append(gall_feat_fc[selected_gall_indices])
        sampled_query_labels.extend([identity] * num_samples)
        sampled_gall_labels.extend([identity] * num_samples)

    return np.vstack(sampled_query_feats), np.vstack(sampled_gall_feats), np.array(sampled_query_labels), np.array(sampled_gall_labels)

# Define function to plot t-SNE visualization
def plot_tsne(query_feats, gall_feats, query_labels, gall_labels, save_path, num_ids=10):
    # return
    # Concatenate the query and gallery features
    combined_feats = np.vstack([query_feats, gall_feats])
    ...
    # Plotting
    plt.figure(figsize=(8, 8))  # Set square figure size
    
    # Plot query features
    for label in np.unique(query_labels):
        indices = np.where(query_labels == label)[0]
        plt.scatter(reduced_feats[indices, 0], reduced_feats[indices, 1], label=f'P{label}', marker='o')

    # Plot gallery features
    for label in np.unique(gall_labels):
        indices = np.where(gall_labels == label)[0]
        plt.scatter(reduced_feats[len(query_feats) + indices, 0], reduced_feats[len(query_feats) + indices, 1], marker='x')

    plt.legend()
    plt.title('t-SNE plot of RGB and IR features', fontsize=14)  # Increase font size
    # plt.xlabel('t-SNE Dimension 1', fontsize=12)  # Increase font size
    # plt.ylabel('t-SNE Dimension 2', fontsize=12)  # Increase font size
    plt.grid(True)

    # Adjust margins to make them softer
...
if dataset == 'sysu':
    ...
    for trial in range(0,num_trials):
        ...
        # if args.mode == 'all':
        #     sampled_query_feats, sampled_gall_feats, sampled_query_labels, sampled_gall_labels = sample_features(query_feat_fc, gall_feat_fc, query_label, gall_label, num_ids = 10)

        #     # Call the function to plot t-SNE visualization
        #     save_path = os.path.join(plots_dir, 'gallery_features_tsne_' + str(trial) + '.png')
        #     plot_tsne(sampled_query_feats, sampled_gall_feats, sampled_query_labels, sampled_gall_labels, save_path, num_ids = 10)
